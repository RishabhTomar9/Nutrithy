...
import { addComment as addCommentService, fetchComments as fetchCommentsService } from '../../services/communityService';
...
export default function CommentSection({ postId, initialComments = [], onCommentAdded }) {
  ...
  const loadComments = async () => {
    try {
      setLoading(true);
      const res = await fetchCommentsService(postId);
  ...
  useEffect(() => {
    if (!initialComments || initialComments.length === 0) loadComments();
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const created = await addCommentService(postId, text.trim(), null);
      // add to local UI
      setComments((p) => [...p, created]);
      setText('');
      // notify parent PostCard to increment comment count
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} className="flex gap-2 items-start">
        <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Write a comment..." className="flex-1 p-2 rounded-md bg-gray-700 text-white" rows={2} disabled={submitting} />
      ...
      <div className="mt-3 space-y-2 max-h-60 overflow-auto">
        {loading ? <div>Loading...</div> : comments.length === 0 ? <div className="text-sm text-gray-400">No comments yet</div> : comments.map((c) => (
